Use the `newsletter-now` skill behavior.

Task:
- Generate the AI newsletter now.
- Use Codex for the editorial pass, not a hardcoded keyword-only formatter.

Required workflow:
1. Read `__RUNTIME_ROOT__/.data/config.json` if it exists.
2. Run `python3 __RUNTIME_ROOT__/scripts/run_all.py` to collect raw items.
3. If there are no items, respond with exactly `새 뉴스 없음`.
4. Curate the raw items:
- keep only meaningful AI news
- remove duplicates and near-duplicates across sources
- drop weak or noisy items
- categorize with judgment into:
  - `🔬 모델 & 리서치`
  - `🛠️ 도구 & 오픈소스`
  - `🔒 보안`
  - `📊 업계 동향`
  - `💻 개발 실무`
- translate English titles into natural Korean
5. Create the digest in this shape:
- header line with current KST time
- category heading
- platform heading
- markdown links `[제목](URL)`
6. If Telegram is enabled in config, send the digest by category using:
- `python3 __RUNTIME_ROOT__/scripts/send_telegram.py`
7. Only after successful Telegram delivery, mark the delivered source items by piping grouped delivered items to:
- `python3 __RUNTIME_ROOT__/scripts/mark_delivered.py`
8. If Telegram is disabled and you print the digest to the terminal instead, still mark the actually included digest items as delivered with the same script.
9. Final answer must be one line only:
- `N개 뉴스 수집, M개 카테고리, Telegram 전송 완료`
- or `N개 뉴스 수집, M개 카테고리 출력`
- or `새 뉴스 없음`

Constraints:
- Do not modify config unless required for normal collector delivery-state updates.
- Do not invent titles, categories, or URLs.
- Prefer concise terminal output.

